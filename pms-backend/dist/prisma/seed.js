...
const cleanChecklist = (c) => ({
    ...
    discipline: c.discipline, // ensure one of: 'Civil' | 'MEP' | 'Finishes'
    stageLabel: String(c.stageLabel || '').trim(),
    tags: Array.isArray(c.tags) ? c.tags : [],
    status: c.status || 'Active',
    version: Number.isFinite(c.version) ? c.version : 1,
});
const cleanChecklistItem = (i, checklistId) => ({
    checklistId,
    seq: Number(i.seq) || 0,
...
async function upsertActivity(tx, a) {
    if (a.code) {
...
async function upsertMaterial(tx, m) {
    if (m.code) {
...
async function upsertChecklist(tx, c) {
...
async function upsertChecklistItem(tx, item) {
...
async function upsertItemActivityLink(tx, itemId, label, activityId, tags = []) {
...
async function upsertItemMaterialLink(tx, itemId, name, materialId, category, properties) {
...
async function ensureActivityMaterial(tx, activityId, materialId, note) {
...
async function main() {
    await prisma.$transaction(async (tx) => {
        ...
        for (const cRaw of checklistsRaw) {
            ...
            const itemsForChecklist = checklistItemsRaw.filter((i) => ciEquals(i.checklistCode, c.code));
            // Upsert items
            for (const iRaw of itemsForChecklist) {
                ...
                for (const sMat of iRaw.suggestedMaterials ?? []) {
                    ...
                    await upsertItemMaterialLink(tx, item.id, m.name, matched?.id ?? null, m.category ?? null, m.properties ?? null);
                }
            }
        }
