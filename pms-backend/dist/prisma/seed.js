...
async function main() {
    ...
    const dl = await prisma.state.upsert({
        where: { code: "DL" },
        update: {},
        create: { code: "DL", name: "Delhi", type: "UT" },
    });
    const mh = await prisma.state.upsert({
        where: { code: "MH" },
        update: {},
        create: { code: "MH", name: "Maharashtra", type: "State" },
    });
    const ka = await prisma.state.upsert({
        where: { code: "KA" },
        update: {},
        create: { code: "KA", name: "Karnataka", type: "State" },
    });
    ...
    const companyRoles = [
        client_1.CompanyRole.IH_PMT,
        client_1.CompanyRole.PMC,
        client_1.CompanyRole.Supplier,
        client_1.CompanyRole.Contractor,
        client_1.CompanyRole.Consultant,
    ];
    const companies = await Promise.all(Array.from({ length: 10 }, async (_, i) => {
    ...
    const clients = await Promise.all(Array.from({ length: 10 }, async (_, i) => {
        const idx = i + 1;
        ...
        return prisma.user.upsert({
            ...
            create: {
                ...
                email,
                isClient: true,
                userStatus: idx % 5 === 0 ? client_1.UserStatus.Inactive : client_1.UserStatus.Active,
                stateId: st.stateId,
                districtId: dt.districtId,
    ...
    const nonClients = await Promise.all(Array.from({ length: 10 }, async (_, i) => {
        ...
        const existing = await prisma.userRoleMembership.findFirst({
            where: {
                userId: u.userId,
                role,
                scopeType: client_1.RoleScope.Company,
