...
export default function ContractorsAssignments() {
    ...
    const filtered = onlyRole.filter((u) => {
      ...
      if (companyFilter) {
        ...
        const companyNames = new Set(
          mems
            .filter(m => String(m?.role || "").toLowerCase() === "contractor") // enforce role
  ...
  const onMoveToTile2 = (user: UserLite) => {
    if (alreadyAssignedToSelectedProject(user, selectedProjectId)) {
      ...
      alert(`${name} has already assigned ${projectTitle}. If you wish to make changes, edit the Contractor Assignments.`);
      return;
    }
  ...
  return (
    <div className="mx-auto max-w-6xl">
            ...
            <div className="mt-2 flex items-center justify-end gap-2">
              <button
                ...
                onClick={() => {
                  ...
                  setSelectedIds(new Set());
                  setMovedIds(new Set());
                  const el = document.querySelector('[data-tile-name="Browse Contrasctor"]');
                  el?.scrollIntoView({ behavior: "smooth", block: "start" });
                }}
              >
                Cancel
              ...
              <button
                ...
                onClick={onAssign}
                disabled={!canSubmit}
                title={canSubmit ? "Assign selected contractors to project" : "Select all required fields"}
              >
      ...
      <section className="bg-white dark:bg-neutral-900 rounded-2xl shadow-sm border dark:border-neutral-800 p-4 mb-4" aria-label="Tile: Browse Contractors" data-tile-name="Browse Contractors">
        ...
        <div className="border rounded-xl dark:border-neutral-800 overflow-hidden">
          <div className="overflow-auto" style={{ maxHeight: "55vh" }}>
            ...
            {usersLoading ? (
              <div className="p-4 text-sm text-gray-600 dark:text-gray-300">Loading contractors…</div>
            ) : rowsPaged.length === 0 ? (
              <div className="p-4 text-sm text-gray-600 dark:text-gray-300">No contractors match the selected criteria.</div>
            ) : (
          ...
          <div className="flex items-center justify-between px-3 py-2 text-xs border-t dark:border-neutral-800">
            <div className="text-gray-600 dark:text-gray-300">
              Page <b>{pageSafe}</b> of <b>{totalPages}</b> · Showing <b>{rowsPaged.length}</b> of <b>{total}</b> contractors
      ...
      <section className="bg-white dark:bg-neutral-900 rounded-2xl shadow-sm border dark:border-neutral-800 p-4" aria-label="Tile: contractors Assignments" data-tile-name="Contractor Assignments">
        <TileHeader title="Tile 4 — contractors Assignments" subtitle="All contractors who have been assigned to projects." />
            ...
            <div className="mt-4 flex justify-end gap-2">
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    // Prefer the company on the membership; fall back to user's Contractor company
                    const companyId =
