// src/views/admin/assignments/suppliers/suppliersAssignments.tsx
...
function projectsLabel(u: UserLite): string {
  ...
  const set = new Set(
    mem
      .filter((m) => String(m?.role || "").toLowerCase() === "supplier")
...
function isSupplierUser(u: UserLite): boolean {
  const mem = Array.isArray(u.userRoleMemberships) ? u.userRoleMemberships : [];
  return mem.some((m) => String(m?.role || "").toLowerCase() === "supplier");
}
...
function supplierCompanyId(u: UserLite): string | null {
  ...
  const m = mems.find(
    (mm) =>
      String(mm?.role || "").toLowerCase() === "supplier" &&
      mm?.company?.companyId
  );
...
// ===== UI constants (CompanyEdit look) — EXACT same as Clients page =====
const CARD =
...
export default function SuppliersAssignments() {
  ...
  // Section 3 (browse suppliers) data + refs
  ...
  // Load users (include memberships)
  useEffect(() => {
    let alive = true;
    (async () => {
      try {
  ...
  const companyOptions = useMemo<string[]>(() => {
    ...
    for (const u of allUsers) {
      const mems = Array.isArray(u.userRoleMemberships) ? u.userRoleMemberships : [];
      for (const m of mems) {
        if (String(m?.role || "").toLowerCase() !== "supplier") continue;
  ...
  const suppliersRows = useMemo<Row[]>(() => {
    const moved = movedIds;

    const onlySuppliers = allUsers
      .filter(isSupplierUser)
      .filter((u) => !moved.has(u.userId));

    const filtered = onlySuppliers.filter((u) => {
      if (statusFilter !== "all") {
  ...
  const total = suppliersRows.length;
  const totalPages = Math.max(1, Math.ceil(total / pageSize));
  const pageSafe = Math.min(Math.max(1, page), totalPages);

  const rowsPaged = useMemo<Row[]>(() => {
    const start = (pageSafe - 1) * pageSize;
    return suppliersRows.slice(start, start + pageSize);
  }, [suppliersRows, pageSafe, pageSize]);
  ...
  const onAssign = async () => {
    ...
    if (dupes.length > 0) {
      const lines = dupes.map((u) => {
        ...
        return `${name} has already assigned ${projectTitle}. If you wish to make changes, edit the Supplier Assignments.`;
      });
      alert(lines.join("\n"));
      return;
    }
    ...
    const summary =
      ...
      `Suppliers: ${names.length ? names.join(", ") : "—"}\n` +
    ...
    try {
      ...
      alert(
        `Assigned ${data?.created ?? items.length} supplier(s) to "${projectTitle}".`
      );
      ...
      // Refresh users list so Section 4 updates immediately
      try {
      ...
      const el = document.querySelector('[data-tile-name="Browse Suppliers"]');
  ...
  // Move user from Section 3 to Section 2
  const onMoveToTile2 = (user: UserLite) => {
    if (alreadyAssignedToSelectedProject(user, selectedProjectId)) {
      ...
      alert(
        `${name} has already assigned ${projectTitle}. If you wish to make changes, edit the Supplier Assignments.`
      );
      return;
    }
  ...
  const movedSuppliersList = useMemo<UserLite[]>(() => {
  ...
  // ---- Section 4 data: flatten "Supplier" memberships with a project
  type AssignmentRow = {
    userId: string;
    userName: string;
    projectId: string;
  ...
  const assignedSupplierRows = useMemo<AssignmentRow[]>(() => {
  ...
  // ===== Section 4 sort state + sorted rows =====
  ...
  const assignedSortedRows = useMemo<AssignmentRow[]>(() => {
    const rows = [...assignedSupplierRows];
    const key = aSortKey;
    const dir = aSortDir;
    const cmp = (a: any, b: any) => {
      if (a === b) return 0;
    ...
    return rows;
  }, [assignedSupplierRows, aSortKey, aSortDir]);

  // ===== Section 4 pagination =====
  const [aPage, setAPage] = useState(1);
  const aPageSize = pageSize; // use same selector as Browse Suppliers
  ...
  const findCurrentMembershipId = async (userId: string, projectId: string) => {
    const pickBest = (mems: any[]) => {
      const candidates = mems
        .filter((mem) => String(mem?.role || "").toLowerCase() === "supplier")
        .filter(
          (mem) =>
  ...
  const _editFromMin = floorForEditFrom(existingFromForMin); // kept for UI parity (even if unused)
  ...
  return (
    <div className="w-full">
      ...
      <div className="mb-4">
        ...
        <div className="mt-1 text-sm text-slate-600 dark:text-slate-300">
          Assign suppliers to projects and manage validity.
        </div>
      ...
      <section className={CARD + " mb-4"} aria-label="Projects" data-tile-name="Projects">
        <SectionHeader title="Projects" subtitle="Choose the project to assign suppliers to." />
      ...
      <section className={CARD + " mb-4"} aria-label="Roles & Options" data-tile-name="Roles & Options">
        <SectionHeader title="Roles & Options" subtitle="Pick from moved suppliers and set validity." />
        ...
        <div className="mt-2 space-y-5">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              {movedSuppliersList.length > 0 && (
                <div className="text-[11px] text-slate-500 dark:text-slate-400">
                  {selectedIds.size}/{movedSuppliersList.length} selected
                </div>
              )}
            </div>
            ...
            <div className="rounded-2xl border border-slate-200 dark:border-white/10 overflow-auto bg-slate-50/60 dark:bg-white/5">
              {movedIds.size === 0 ? (
                <div className="p-3 text-sm text-slate-600 dark:text-slate-300">
                  <b>Move suppliers</b> from the list below to assign them for the selected project.
                </div>
              ) : (
                <ul className="divide-y divide-slate-200 dark:divide-white/10">
                  {movedSuppliersList.map((u) => {
            ...
            {movedSuppliersList.length > 0 && (
              <div className="flex flex-wrap gap-2">
                <button
                  ...
                  onClick={() =>
                    setSelectedIds(new Set(movedSuppliersList.map((m) => m.userId)))
                  }
                >
                  Select All
                </button>
                <button className={BTN_SECONDARY} onClick={() => setSelectedIds(new Set())}>
                  Clear Selection
                </button>
              </div>
            )}
          ...
          <div className="space-y-3">
            ...
            <div className="mt-1 flex items-center justify-end gap-2">
              <button
                ...
                title="Clear dates and move suppliers back to Browse Suppliers"
              >
                Cancel
              </button>
              ...
              <button
                ...
                title={
                  canSubmit
                    ? "Assign selected suppliers to project"
                    : "Select project, suppliers and validity dates"
                }
              >
      ...
      {/* Section 3 — Browse Suppliers */}
      <section className={CARD + " mb-4"} aria-label="Browse Suppliers" data-tile-name="Browse Suppliers">
        <SectionHeader
          title="Browse Suppliers"
          subtitle="Search, filter, sort and move suppliers into the selection."
        />
        ...
        <div className="mt-4 rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden dark:border-white/10 dark:bg-neutral-950">
                    ...
                    >
                      ...
                      <td className="px-2 py-1.5 whitespace-nowrap align-middle">
                        <button
                          ...
                          title="Move this supplier to selection"
                          aria-label="Move this supplier to selection"
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 px-3 py-2 text-sm border-t border-slate-200 dark:border-white/10">
            <div className="text-slate-600 dark:text-slate-300">
              ...
              <b>{rowsPaged.length}</b> of <b>{total}</b> suppliers
              {stateFilter ? (
                <>
                  {" "}
      ...
      {/* Section 4 — Supplier Assignments */}
      <section className={CARD + " mb-4"} aria-label="Supplier Assignments" data-tile-name="Supplier Assignments">
        <SectionHeader
          title="Supplier Assignments"
          subtitle="All suppliers who have been assigned to projects."
        />
        ...
        <div className="mt-4 rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden dark:border-white/10 dark:bg-neutral-950">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 px-3 py-2 text-sm border-t border-slate-200 dark:border-white/10">
            <div className="text-slate-600 dark:text-slate-300">
              ...
              <b>{assignedRowsPaged.length}</b> of <b>{aTotal}</b> supplier assignments
            </div>
