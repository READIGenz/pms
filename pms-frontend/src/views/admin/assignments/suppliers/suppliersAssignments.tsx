...
// ---------- UI atoms (match Consultant page) ----------
...
export default function SuppliersAssignments() {
  ...
  // Sorting (same defaults as Consultant)
  ...
  // ===== Assignments pagination state (declare early so handlers can use setAPage safely) =====
    ...
    for (const u of allUsers) {
      ...
      for (const m of mems) {
        ...
        const vf = fmtLocalDateOnly(
          m.validFrom ?? (m as any).validFromDate ?? (m as any).from ?? (m as any).startDate ??
          (m as any).end ?? (m as any).valid_from ?? (m as any).validFromAt ?? (m as any).valid_from_at
        );
        const vt = fmtLocalDateOnly(
          m.validTo ?? (m as any).validToDate ?? (m as any).to ?? (m as any).endDate ??
          (m as any).end ?? (m as any).valid_to ?? (m as any).validToAt ?? (m as any).valid_to_at
        );
  ...
  useEffect(() => { if (aPage > aTotalPages) setAPage(aTotalPages); }, [aTotalPages, aPage]);
  ...
  return (
    <div className="mx-auto max-w-6xl">
      ...
      <section className={TILE_SHELL} aria-label="Tile: Browse Suppliers" data-tile-name="Browse Suppliers">
        <TileHeader
          title="Browse Suppliers"
          subtitle="Search and filter; sort columns; paginate. Use the up arrow to move suppliers to Tile 2."
        />
          ...
          <div className="flex items-center justify-between px-3 py-2 text-xs border-t dark:border-neutral-800 bg-white dark:bg-neutral-900">
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                onClick={() => setPage(1)} disabled={pageSafe <= 1} title="First"
              >
                « First
              </button>
              <button
                className="px-3 py-1 rounded-full border border-slate-200 dark:border-neutral-800 disabled:opacity-50"
                onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={pageSafe <= 1} title="Previous"
              >
                ‹ Prev
              </button>
              <button
                className="px-3 py-1 rounded-full border border-slate-200 dark:border-neutral-800 disabled:opacity-50"
                onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={pageSafe >= totalPages} title="Next"
              >
                Next ›
              </button>
              <button
                className="px-3 py-1 rounded-full border border-slate-200 dark:border-neutral-800 disabled:opacity-50"
                onClick={() => setPage(totalPages)} disabled={pageSafe >= totalPages} title="Last"
              >
                Last »
              </button>
            </div>
      ...
      {/* Tile 4 — Supplier Assignments */}
      <section
        className={TILE_SHELL.replace("mb-4", "")}
        aria-label="Tile: Supplier Assignments"
        data-tile-name="Supplier Assignments"
      >
        <TileHeader
