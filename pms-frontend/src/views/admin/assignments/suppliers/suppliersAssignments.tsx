...
export default function SuppliersAssignments() {
  ...
  const companyOptions = useMemo<string[]>(() => {
    ...
    for (const u of allUsers) {
      const mems = Array.isArray(u.userRoleMemberships) ? u.userRoleMemberships : [];
      for (const m of mems) {
        // Only include companies where the membership role is suppliers
  ...
  const onHardDeleteFromEdit = async () => {
    ...
    try {
      ...
      if (status === 404) {
        ...
        alert(errMsg); // keep modal open so user can retry
      }
    } finally {
      setDeleting(false);
    }
  ...
  const rowsAll = useMemo<Row[]>(() => {
    ...
    const onlyRole = allUsers
      .filter(u => isRoleUser(u, "Supplier"))
    ...
    const filtered = onlyRole.filter((u) => {
      ...
      if (companyFilter) {
        ...
        const companyNames = new Set(
          mems
            .filter(m => String(m?.role || "").toLowerCase() === "supplier") // enforce role
  ...
  const onAssign = async () => {
    ...
    try {
      ...
      alert(`Assigned ${data?.created ?? items.length} supplier(s) to "${projectTitle}".`);
  ...
  const onMoveToTile2 = (user: UserLite) => {
    if (alreadyAssignedToSelectedProject(user, selectedProjectId)) {
      ...
      alert(`${name} has already assigned ${projectTitle}. If you wish to make changes, edit the Supplier Assignments.`);
      return;
    }
  ...
  // ---- Tile 4 data: flatten Supplier assignments
  type AssignmentRow = {
    userId: string;
    userName: string;
    projectId: string;
  ...
  /** Try to find the freshest Supplier membership id for (userId, projectId) */
  ...
  return (
    <div className="mx-auto max-w-6xl">
      <div className="mb-6">
        <h1 className="text-2xl font-semibold dark:text-white">Supplier Assignments</h1>
        <p className="text-sm text-gray-600 dark:text-gray-300">
          Projects · Roles & Options · <b>Browse Suppliers</b> · Supplier Assignments
        </p>
      ...
      <section className="bg-white dark:bg-neutral-900 rounded-2xl shadow-sm border dark:border-neutral-800 p-4 mb-4" aria-label="Tile: Roles & Options" data-tile-name="Roles & Options">
        <TileHeader title="Roles & Options" subtitle="Pick from moved suppliers & set validity." />

        <div className="mt-4 grid grid-cols-1 lg:grid-cols-2 gap-4">
          {/* moved list */}
          <div className="space-y-3" aria-label="Subtile: Moved Suppliers">
            <label className="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">
              Moved Suppliers (select with checkbox)
            </label>

            <div className="border rounded-lg dark:border-neutral-800 overflow-auto" style={{ maxHeight: 300 }}>
              {movedIds.size === 0 ? (
                <div className="p-3 text-sm text-gray-600 dark:text-gray-300">
                  <b>Move Suppliers</b> from list below to assign roles.
                </div>
              ) : (
          ...
          <div className="space-y-3" aria-label="Subtile: Validity">
            ...
            <div className="mt-2 flex items-center justify-end gap-2">
              ...
              <button
                ...
                onClick={onAssign}
                disabled={!canSubmit}
                title={canSubmit ? "Assign selected suppliers to project" : "Select all required fields"}
              >
      ...
      {/* Tile 3 — Browse Suppliers */}
      <section className="bg-white dark:bg-neutral-900 rounded-2xl shadow-sm border dark:border-neutral-800 p-4 mb-4" aria-label="Tile: Browse Suppliers" data-tile-name="Browse Suppliers">
        <TileHeader title="Browse Suppliers" subtitle="Search and filter; sort columns; paginate. Use ‘Move’ to add suppliers to Tile 2." />
        ...
        <div className="border rounded-xl dark:border-neutral-800 overflow-hidden">
          <div className="overflow-auto" style={{ maxHeight: "55vh" }}>
            ...
            {usersLoading ? (
              <div className="p-4 text-sm text-gray-600 dark:text-gray-300">Loading suppliers…</div>
            ) : rowsPaged.length === 0 ? (
              <div className="p-4 text-sm text-gray-600 dark:text-gray-300">No suppliers match the selected criteria.</div>
            ) : (
                    ...
                    <tr key={r._id} className="odd:bg-gray-50/50 dark:odd:bg-neutral-900/60">
                      <td className="px-3 py-2 border-b dark:border-neutral-800 whitespace-nowrap">
                        <button
                          ...
                          title="Move this suppliers to selection"
          ...
          <div className="flex items-center justify-between px-3 py-2 text-xs border-t dark:border-neutral-800">
            <div className="text-gray-600 dark:text-gray-300">
              Page <b>{pageSafe}</b> of <b>{totalPages}</b> · Showing <b>{rowsPaged.length}</b> of <b>{total}</b> suppliers
      ...
      {/* Tile 4 — suppliers Assignments */}
      <section className="bg-white dark:bg-neutral-900 rounded-2xl shadow-sm border dark:border-neutral-800 p-4" aria-label="Tile: suppliers Assignments" data-tile-name="Supplier Assignments">
        <TileHeader title="Suppliers Assignments" subtitle="All suppliers who have been assigned to projects." />

        <div className="border rounded-xl dark:border-neutral-800 overflow-hidden">
          <div className="overflow-auto" style={{ maxHeight: "55vh" }}>
            {assignedSortedRows.length === 0 ? (
              <div className="p-4 text-sm text-gray-600 dark:text-gray-300">No suppliers assignments found.</div>
            ) : (
              <table className="min-w-full text-sm">
                <thead className="bg-gray-50 dark:bg-neutral-800 sticky top-0 z-10">
                  <tr>
          ...
          <div className="flex items-center justify-between px-3 py-2 text-xs border-t dark:border-neutral-800">
            <div className="text-gray-600 dark:text-gray-300">
              Page <b>{aPageSafe}</b> of <b>{aTotalPages}</b> · Showing <b>{assignedRowsPaged.length}</b> of <b>{aTotal}</b> supplier assignments
            </div>
            <div className="flex items-center gap-1">
              <button
      ...
      {viewOpen && viewRow && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white dark:bg-neutral-900 rounded-2xl shadow-lg border dark:border-neutral-800 w-full max-w-md p-4">
            <div className="text-lg font-semibold mb-2 dark:text-white">Suppliers Assignment</div>
              ...
              <table className="min-w-full text-sm">
                <tbody>
                  <tr className="odd:bg-gray-50/60 dark:odd:bg-neutral-900/60">
                    <td className="px-3 py-2 font-medium whitespace-nowrap">Suppliers</td>
