// pms-frontend/src/views/pmc/pmcHome.tsx
import { useEffect } from 'react';
import { Outlet, useLocation, Navigate, NavLink } from 'react-router-dom';

export default function pmcHome() {
  // client-side guard
  const token = localStorage.getItem('token');
  const loc = useLocation();

  useEffect(() => {
    document.title = 'Trinity PMS — PMC';
  }, []);
      ...
      <header className="w-full px-4 sm:px-6 lg:px-10 py-4 border-b dark:border-neutral-800">
        <div className="mx-auto max-w-7xl flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-xl font-bold tracking-tight dark:text-white">Trinity PMS — PMC</div>
