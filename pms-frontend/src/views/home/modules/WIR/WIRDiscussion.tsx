...
export default function WIRDiscussion({ wirId, wirCode, creatorName }: Props) {
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div
        ref={listRef}
        className="rounded-2xl border border-slate-200/80 dark:border-white/10 bg-slate-50/60 dark:bg-white/[0.03] p-3 sm:p-4 max-h-[50vh] overflow-auto shadow-sm"
      >
        {loading ? (
          <div className="text-sm text-slate-700 dark:text-neutral-200">Loading…</div>
        ) : err ? (
          <div className="text-sm text-rose-600">{err}</div>
        ) : rows.length === 0 ? (
          <div className="text-sm text-slate-600 dark:text-neutral-300">No messages yet.</div>
        ) : (
          <ul className="space-y-3">
            {rows.map((m) => (
              <li
                key={m.id}
                className="rounded-2xl bg-white dark:bg-neutral-950 border border-slate-200/80 dark:border-white/10 p-3 shadow-sm"
              >
                ...
                <div className="mt-1 text-sm whitespace-pre-wrap break-words text-slate-900 dark:text-white">
                  {m.text}
                </div>
              </li>
            ))}
      ...
      {/* Composer */}
      <div className="rounded-2xl border border-slate-200/80 dark:border-white/10 bg-white dark:bg-neutral-950 p-3 sm:p-4 shadow-sm">
        <div className="text-[12px] text-slate-600 dark:text-neutral-300 mb-2">
        ...
        <textarea
          ...
          onKeyDown={onKeyDown}
          rows={3}
          className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 shadow-sm outline-none resize-none
                     focus:ring-2 focus:ring-[#00379C]/25 focus:border-[#00379C]/40
                     dark:bg-neutral-950 dark:border-white/10 dark:text-white dark:focus:ring-[#FCC020]/25"
          ...
          <div className="flex flex-wrap items-center gap-2 justify-end">
            <button
              ...
              className="inline-flex items-center justify-center rounded-full border border-slate-200 bg-white px-4 py-2 text-xs sm:text-sm font-medium text-slate-700 shadow-sm
                         ...
                         dark:bg-neutral-950 dark:border-white/10 dark:text-neutral-100 dark:hover:bg-white/[0.03]"
            ...
            <button
              ...
              className="inline-flex items-center justify-center rounded-full bg-[#00379C] px-5 py-2 text-xs sm:text-sm font-semibold text-white shadow-sm
