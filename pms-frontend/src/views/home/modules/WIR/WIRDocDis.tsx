...
function TabButton({
  ...
  return (
    <button
      ...
      type="button"
      className={
        "inline-flex items-center justify-center rounded-full border px-4 py-2 text-xs sm:text-sm font-medium shadow-sm transition " +
        (active
          ? "bg-emerald-600 text-white border-emerald-600 hover:bg-emerald-700 dark:border-emerald-700"
...
export default function WIRDocDis() {
  ...
  const addPendingFiles = (itemId: string, files: FileList | File[]) => {
    ...
    const saved =
      row?.items?.find((x) => x.id === itemId)?.evidences?.length || 0;
    const current = edits[itemId]?.pendingFiles?.length || 0;
    const room = Math.max(0, MAX_EVIDENCES_PER_ITEM - (saved + current));
    if (room <= 0) {
      setErr(
        `You can attach up to ${MAX_EVIDENCES_PER_ITEM} files for this item.`
      );
      return;
    }
    ...
    setEdits((m) => {
      ...
      return {
        ...m,
        [itemId]: { ...(m[itemId] || {}), pendingFiles: [...prev, ...toAdd] },
      };
    });
  };
  ...
  const deleteSavedEvidence = async (itemId: string, evidenceId: string) => {
    ...
    try {
      ...
      // rollback on failure
      setErr(
        e?.response?.data?.error || e?.message || "Failed to delete attachment."
      );
  ...
  const [recSubmitting, setRecSubmitting] = useState<
    null | "APPROVE" | "APPROVE_WITH_COMMENTS" | "REJECT"
  >(null);

  // HOD finalize submit state (new, accepts "ACCEPT" | "REJECT")
  const [finalizeSubmitting, setFinalizeSubmitting] = useState<
    null | "ACCEPT" | "REJECT"
  >(null);

  // Hold local (unsaved) pick
  const [pendingRec, setPendingRec] = useState<
    WirDoc["inspectorRecommendation"] | null
  >(null);
  ...
  const uploadEvidence = useCallback(
    async (itemId: string, file: File) => {
      ...
      try {
        ...
        const kind =
          (first.kind as WirItemEvidence["kind"]) || detectKind(file);
    ...
    for (const [itemId, file] of pairs) {
      try {
        ...
        setEdits((m) => {
          ...
          return {
            ...m,
            [itemId]: { ...(m[itemId] || {}), pendingFiles: next },
          };
        });
      } catch {
  ...
  const computeCriticalFail = useCallback(() => {
    ...
    for (const it of row.items) {
      const isCritical =
        !!it.critical ||
        (it.tags?.some((t) => /^\s*critical\s*$/i.test(t)) ?? false);
  ...
  const [hodSelectedUserId, setHodSelectedUserId] = useState<string | null>(
    null
  );
  ...
  const onConfirmSendToHod = useCallback(async () => {
    ...
    const finalRec: "APPROVE" | "APPROVE_WITH_COMMENTS" | "REJECT" = (
      recLockedReject ? "REJECT" : pendingRec ?? "APPROVE"
    ) as any;
  ...
  const validateAllRunnerFields = useCallback(() => {
    if (!row?.items?.length)
      return { ok: false, missing: [] as Array<{ id: string; name: string }> };
    ...
    for (const it of row.items) {
      ...
      if (!hasStatus) {
        missing.push({
          id: it.id,
          name: it.name || it.code || "Item (status)",
        });
      ...
      if (needsMeasurement) {
        ...
        if (!hasNumeric) {
          missing.push({
            id: it.id,
            name: it.name || it.code || "Item (measurement)",
          });
          continue;
        }
      }
      ...
      if (needsEvidence) {
        ...
        if (savedCount + pendingCount === 0) {
          missing.push({
            id: it.id,
            name: it.name || it.code || "Item (evidence/document)",
          });
          continue;
        }
      }
  ...
  const loadHodDerived = useCallback(async () => {
    ...
    try {
      ...
      const bag = new Map<
        string,
        { userId: string; roleKey: RoleKey; u: any }
      >();
      for (const rk of ROLE_KEYS) {
        ...
        for (const { user } of rows) {
          ...
          if (!bag.has(uid))
            bag.set(uid, { userId: uid, roleKey: rk, u: user });
        }
      }
      ...
      setHodErr(
        e?.response?.data?.error || e?.message || "Failed to derive HOD users."
      );
      setHodList([]);
    } finally {
      setHodLoading(false);
  ...
  useEffect(() => {
    ...
    (async () => {
      if (!row) {
        if (alive) setCanSeeRunner(false);
        return;
      }
      ...
      const userId = String(
        (claims as any)?.userId || (user as any)?.userId || ""
      );
      ...
      try {
        ...
        if (alive)
          setActingRole(
            (["Inspector", "HOD", "Inspector+HOD"] as const).includes(
              acting as any
            )
              ? (acting as any)
              : null
        ...
        const ok =
          acting === "Inspector" ||
          acting === "HOD" ||
          acting === "Inspector+HOD" ||
          contractorStatusOk;
        if (alive) setCanSeeRunner(ok);
      } catch {
        if (alive) {
          setCanSeeRunner(false);
          setActingRole(null);
        }
      }
    })();
  ...
  const isFinalized = useMemo(() => {
    ...
    return (
      st === "Approved" || st === "Rejected" || st === "APPROVE_WITH_COMMENTS"
    );
  }, [row?.status]);
  ...
  const combinedItemsCount = useMemo(() => {
    ...
    const fromChecklists = (row?.checklists ?? []).reduce(
      (sum, c) =>
        sum +
        (typeof c.itemsCount === "number"
          ? c.itemsCount
          : typeof c.itemsTotal === "number"
          ? c.itemsTotal
          : 0),
      0
    );
  ...
  const mandatoryCount = useMemo(() => {
    ...
    return row.items.filter((it) => /^mandatory$/i.test((it.spec || "").trim()))
      .length;
  }, [row?.items]);
  ...
  const registerFileRef =
    (itemId: string, index: number) => (el: HTMLInputElement | null) => {
  ...
  const [sendWarnList, setSendWarnList] = useState<
  ...
  const [hodList, setHodList] = useState<
    Array<{ userId: string; fullName: string; acting: "HOD" | "Inspector+HOD" }>
  >([]);
  const [hodConfirmOpen, setHodConfirmOpen] = useState(false);

  const [actingRole, setActingRole] = useState<
    "Inspector" | "HOD" | "Inspector+HOD" | null
  >(null);
  ...
  const showReschedCta = useMemo(() => {
    ...
    const isFinal = (() => {
      const st = canonicalWirStatus(row?.status);
      return (
        st === "Approved" || st === "APPROVE_WITH_COMMENTS" || st === "Rejected"
      );
    })();
    const isBicSelf = !!row?.bicUserId && String(row.bicUserId) === currentUid;
  ...
  const [finalizeOutcome, setFinalizeOutcome] = useState<
    "ACCEPT" | "REJECT" | null
  >(null);
  ...
  useEffect(() => {
    ...
    if (finalizeOpen || row?.inspectorId)
      loadInspectorName((row as any)?.inspectorId);
    return () => {
      alive = false;
    };
    ...
    (async () => {
      try {
        ...
        const { data } = await api.get("/admin/users", {
          params: { includeMemberships: 0 },
        });
        const users: any[] = Array.isArray(data?.users)
          ? data.users
          : Array.isArray(data)
          ? data
          : [];
  ...
  useEffect(() => {
    ...
    const ids = [
      hodPlannedPatch.hodId,
      hodPlannedPatch.bicUserId,
    ...
    (async () => {
      for (const id of missing) {
        try {
          ...
          const u = (data?.user ?? data) || {};
          if (!ignore)
            setActorNameMap((prev) => ({ ...prev, [id]: displayNameLite(u) }));
        } catch {
          try {
            const { data } = await api.get(`/users/${id}`);
            const u = (data?.user ?? data) || {};
  ...
  return (
    <section className="bg-white dark:bg-neutral-900 rounded-2xl shadow-sm border dark:border-neutral-800 p-4 sm:p-5 md:p-6">
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <button
              ...
              className="shrink-0 inline-flex items-center justify-center h-4 w-4 rounded-full border border-slate-200 bg-white text-slate-700 shadow-sm
      ...
      {/* Content area */}
      <div className="mt-4 rounded-2xl bg-emerald-50/60 border border-emerald-200/80 dark:bg-emerald-900/20 dark:border-emerald-800/40 p-3 sm:p-5">
        {loading ? (
          ...
          <>
            ...
            {subtab === "overview" ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div className="rounded-2xl border border-slate-200/80 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-4 shadow-sm">
                  ...
                  <div className="text-sm dark:text-white space-y-1">
                    ...
                    <div>
                      <b>Discipline:</b> {row.discipline || "—"}
                    </div>
                    <div>
                      <b>Planned:</b>{" "}
                      {row.forDate
                        ? new Date(row.forDate).toLocaleDateString()
                        : "—"}
                    ...
                    {(row.rescheduleForDate || row.rescheduleForTime) && (
                      <div>
                        ...
                        {row.rescheduleForDate
                          ...
                          : "—"}
                        {row.rescheduleForTime
                          ? ` • ${fmtTime12(row.rescheduleForTime)}`
                          : ""}
                      </div>
                    )}
                    <div>
                      <b>Location:</b> {row.cityTown || "—"}
                    </div>
                    <div>
                      <b>Version:</b>{" "}
                    ...
                    <div>
                      <b>Checklists:</b> {combinedItemsCount} items •{" "}
                      {mandatoryCount} mandatory • {criticalCount} critical
                    </div>
                    <div>
                      <b>Inspector of Record:</b> {inspName || "—"}
                    </div>
                    <div>
                      <b>Ball in Court:</b> {bicName || "—"}
                    </div>{" "}
                  </div>
                  {/* WIR History link */}
                  <div className="pt-2 flex items-center gap-2 flex-wrap">
                    ...
                    >
                      View WIR History
                      {typeof row.histories?.length === "number"
                        ? ` (${row.histories.length})`
                        : ""}
                    </button>
                    ...
                    >
                      View Runner History
                      {totalRunnerRuns ? ` (${totalRunnerRuns})` : ""}
                    </button>
                  </div>
                </div>
                ...
                {canReschedule && (
                  <div className="md:col-span-2 flex justify-end">
                    <button
                      ...
                      className="inline-flex items-center justify-center rounded-full bg-amber-600 px-4 py-2 text-xs sm:text-sm font-semibold text-white shadow-sm
                                                       hover:bg-amber-700 focus:outline-none focus:ring-2 focus:ring-amber-500/40
                      ...
                      <div className="text-sm dark:text-white space-y-2">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[12px] text-gray-700 dark:text-gray-200">
                            ...
                            <b>{row.inspectorRecommendation || "—"}</b>
                          </span>
                        </div>
                        <div className="pt-1">
                          <button
                  ...
                  <div className="rounded-xl border dark:border-neutral-800 p-3 md:col-span-2">
                    ...
                    <div className="text-sm dark:text-white space-y-1">
                      ...
                      <div>
                        <b>HOD Decided At:</b>{" "}
                        {row.hodDecidedAt
                          ? new Date(row.hodDecidedAt).toLocaleString()
                          : "—"}
                      </div>
                    </div>
              ...
              <div className="rounded-2xl border border-slate-200/80 dark:border-neutral-800 bg-white dark:bg-neutral-900 shadow-sm p-4 sm:p-5">
                {/* Tile 1: Runner instruction */}
                <div className="rounded-2xl bg-yellow-50/60 border border-yellow-200/80 dark:bg-emerald-900/20 dark:border-emerald-800/40 shadow-sm p-4 sm:p-5 mb-3">
                  <div className="text-[11px] uppercase tracking-wide text-gray-500 dark:text-gray-400">
                    Runner Checklist
                  </div>
                  <div className="text-sm dark:text-white mt-1">
                    Complete all mandatory items, attach required evidence, and
                ...
                <div className="mb-3 flex items-center justify-end">
                  <button
                    ...
                    disabled={savingAll}
                    className="inline-flex items-center justify-center rounded-full bg-emerald-600 px-5 py-2.5 text-xs sm:text-sm font-semibold text-white shadow-sm
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                    {items.map((it) => {
                      ...
                      return (
                        <div
                          key={it.id}
                          className="rounded-2xl border border-slate-200/80 dark:border-neutral-800 bg-white dark:bg-neutral-900 shadow-sm p-4 sm:p-5 space-y-4"
                        >
                          {/* Tile 2: Item meta */}
                          <div>
           ...
           dark:bg-neutral-800 dark:border-neutral-700 dark:text-neutral-200"
                              ...
                              {(it.evidences?.length || 0) > 0 && (
                                <div className="flex flex-wrap gap-2">
                                  {it.evidences!.map((ev) => (
                                    <span
                                      key={ev.id}
                                      className="inline-flex items-center gap-2 max-w-[320px] truncate text-[12px] px-2 py-1 rounded-lg border dark:border-neutral-800 bg-gray-50 dark:bg-neutral-800 dark:text-gray-100"
                                    >
                                      <a
                                        href={ev.url}
                                        target="_blank"
                                      ...
                                      <span className="text-[10px] px-1.5 py-0.5 rounded border dark:border-neutral-700">
                                        {ev.kind}
                                      </span>
                                      <button
                                        type="button"
                                        onClick={() =>
                                          deleteSavedEvidence(it.id, ev.id)
                                        }
                                    ...
                                    <div className="mt-2 flex items-center gap-2">
                                      {canAdd ? (
                                        <>
                                          {/* Take Photo (camera) */}
                                          <label
                                            className="inline-flex items-center justify-center rounded-full border border-slate-200 bg-white px-3 py-1.5 text-[11px] sm:text-xs font-medium text-slate-700 shadow-sm
                   hover:bg-slate-50 hover:border-slate-300
                   dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-100 dark:hover:bg-neutral-800"
                                          >
                                            Take Photo
                 ...
                 dark:bg-neutral-800 dark:border-neutral-700 dark:text-neutral-200"
                                        >
                                          Max {MAX_EVIDENCES_PER_ITEM}{" "}
                                          attachments reached
                                        </span>
                                      )}

                                      {/* counter -> push to right, stays same line */}
                                      <span
                                        className="ml-auto inline-flex items-center rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-[11px] font-semibold text-slate-700
               dark:bg-neutral-800 dark:border-neutral-700 dark:text-neutral-200"
                                      >
                                    ...
                                    {staged.length > 0 && (
                                      <div className="flex flex-wrap gap-2">
                                        {staged.map((f, idx) => (
                                          <span
                                            key={`${f.name}-${idx}`}
                                            className="inline-flex items-center gap-2 max-w-[260px] truncate text-[12px] px-3 py-1.5 rounded-full
           bg-emerald-50/60 border border-emerald-200/80 text-slate-800 shadow-sm
           dark:bg-emerald-900/20 dark:border-emerald-800/40 dark:text-neutral-100"
                                          >
                                            <span
                                              className="truncate"
                                              title={f.name}
                                            ...
                                            <span
                                              className="text-[10px] px-2 py-0.5 rounded-full border border-emerald-200/80 bg-white/70 text-emerald-800
           dark:bg-neutral-900/40 dark:border-emerald-800/40 dark:text-emerald-200"
                                            >
                                              Pending
                                            </span>
                                            <button
                                              type="button"
                                              onClick={() =>
                                                removePendingFile(it.id, idx)
                                              }
                                              className="ml-1 inline-flex items-center justify-center h-5 w-5 rounded-full border border-slate-200 bg-white text-[11px] text-slate-700
           hover:bg-slate-50 hover:border-slate-300
           dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-100 dark:hover:bg-neutral-800"
                            ...
                            <div>
                              ...
                              <input
                                ...
                                value={buf.value ?? ""}
                                onChange={(e) =>
                                  setEdit(it.id, { value: e.target.value })
                                }
                                className="w-full rounded-full border border-slate-200 bg-white px-4 py-2.5 text-sm text-gray-900 shadow-sm outline-none
           ...
           dark:bg-neutral-900 dark:border-neutral-700 dark:text-white"
                            ...
                            <div className="flex items-center gap-2">
                              <button
                                type="button"
                                onClick={() =>
                                  setEdit(it.id, { status: "PASS" })
                                }
                                className={
                                  "inline-flex items-center justify-center rounded-full border px-3 py-1.5 text-[11px] sm:text-xs font-semibold shadow-sm transition " +
                                  (buf.status === "PASS"
                                    ? "bg-emerald-600 text-white border-emerald-600 hover:bg-emerald-700"
                                    : "bg-white text-emerald-700 border-emerald-200 hover:bg-emerald-50 dark:bg-neutral-900 dark:border-emerald-900/50 dark:text-emerald-200 dark:hover:bg-neutral-800")
                                }
                              >
                                Mark Pass
                              </button>
                              ...
                              <button
                                type="button"
                                onClick={() =>
                                  setEdit(it.id, { status: "FAIL" })
                                }
                                className={
                                  "inline-flex items-center justify-center rounded-full border px-3 py-1.5 text-[11px] sm:text-xs font-semibold shadow-sm transition " +
                                  (buf.status === "FAIL"
                                    ? "bg-rose-600 text-white border-rose-600 hover:bg-rose-700"
                                    : "bg-white text-rose-700 border-rose-200 hover:bg-rose-50 dark:bg-neutral-900 dark:border-rose-900/50 dark:text-rose-200 dark:hover:bg-neutral-800")
                                }
                              >
                                Mark Fail
                              </button>
           ...
           dark:bg-neutral-900 dark:border-neutral-700 dark:text-white"
                ...
                <div className="mt-4 rounded-2xl border dark:border-neutral-800 p-3">
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    <button
                      ...
                      className={
                        "inline-flex items-center justify-center rounded-full border px-4 py-2 text-xs sm:text-sm font-semibold shadow-sm transition " +
                        (rec === "APPROVE"
                          ? "bg-emerald-600 text-white border-emerald-600 hover:bg-emerald-700"
                          : "bg-white text-slate-700 border-slate-200 hover:bg-slate-50 hover:border-slate-300 dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-100 dark:hover:bg-neutral-800")
                      }
                    >
                      Approve
                    </button>
                ...
                <div className="mt-3 flex flex-wrap gap-2 justify-end">
                  <button
                    ...
                    className="inline-flex items-center justify-center rounded-full border border-emerald-600 bg-emerald-600 px-4 py-2 text-xs sm:text-sm font-semibold text-white shadow-sm
               ...
               dark:border-emerald-500 dark:bg-emerald-600 dark:hover:bg-emerald-500"
                  ...
                  <button
                    onClick={onPreview}
                    disabled={savingAll || !!recSubmitting}
                    className="inline-flex items-center justify-center rounded-full border border-slate-200 bg-white px-4 py-2 text-xs sm:text-sm font-semibold text-slate-700 shadow-sm
               ...
               dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-100 dark:hover:bg-neutral-800"
                  ...
                  <button
                    ...
                    disabled={savingAll || !!recSubmitting}
                    className="inline-flex items-center justify-center rounded-full border border-blue-600 bg-blue-600 px-4 py-2 text-xs sm:text-sm font-semibold text-white shadow-sm
               hover:bg-blue-700 disabled:opacity-60
               dark:border-blue-500 dark:bg-blue-600 dark:hover:bg-blue-500"
                  >
          ...
          <div className="absolute inset-0 p-3 sm:p-4 md:p-6">
            <div className="bg-white dark:bg-neutral-900 rounded-2xl shadow-2xl border dark:border-neutral-800 w-full h-full flex flex-col">
              ...
              <div className="flex items-center justify-between p-3 sm:p-4 border-b dark:border-neutral-800">
                ...
                <button
                  className="text-sm px-3 py-2 rounded-full border border-slate-200 bg-white hover:bg-slate-50 hover:border-slate-300
                                           dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-100 dark:hover:bg-neutral-800"
              ...
              <div className="flex-1 overflow-auto p-3 sm:p-4 md:p-6 text-sm leading-6 dark:text-white">
                <section className="space-y-6 max-w-3xl">
                  <div>
                    ...
                    <ul className="list-disc pl-5 space-y-1">
                      <li>
                        The WIR’s status is <b>Submitted</b> or{" "}
                        <b>Recommended</b>, <i>or</i> it is
                        <b> Approved</b>/<b>Rejected</b> (finalized shows only
                        for <b>Inspector</b>, <b>HOD</b>, or{" "}
                        <b>Inspector+HOD</b>).
                      </li>
                      <li>
                        You are viewing as someone who can act as{" "}
                        <b>Inspector</b> or <b>HOD</b> (includes{" "}
                        <b>Inspector+HOD</b>).
                      </li>
                      <li>
                        If any of the above isn’t true, the Runner tab won’t
                        show.
                      </li>
                    </ul>
                  </div>
                  ...
                  <div>
                    ...
                    <ul className="list-disc pl-5 space-y-1">
                      <li>
                        <b>
                          Inspector or Inspector+HOD + Submitted + BIC is you
                        </b>{" "}
                        → opens the <b>Runner items</b> (editable).
                      </li>
                      <li>
                        <b>
                          Inspector or Inspector+HOD + Submitted + BIC is
                          someone else
                        </b>{" "}
                        → shows the <b>Runner — Edit Not Allowed</b> dialog.
                      </li>
                      <li>
                        <b>Inspector or Inspector+HOD + Recommended</b> → opens{" "}
                        <b>Inspector Review</b> (read-only summary modal).
                      </li>
                      <li>
                        <b>HOD + Recommended</b> → opens <b>HOD Review</b>{" "}
                        (read-only summary modal).
                      </li>
                      <li>
                        <b>
                          Inspector or HOD or Inspector+HOD + Rejected/Approved{" "}
                        </b>{" "}
                        → opens <b>HOD Review</b> (read-only summary modal).
                      </li>
                      <li>
                        All other combinations → no action here (use the visible
                        tiles on the screen).
                      </li>
                    </ul>
                  </div>
                  ...
                  <div>
                    <div className="text-[11px] uppercase tracking-wide text-gray-500 dark:text-gray-400 mb-1">
                      Runner items (what you need to fill)
                    </div>
                    <p className="mb-2">
                      For each item you want to include in a send:
                    </p>
                    <ul className="list-disc pl-5 space-y-1">
                      <li>
                        Enter a <b>numeric measurement</b> (e.g., 10 or 10.5).
                      </li>
                      <li>
                        Choose <b>Pass</b> or <b>Fail</b>.<br />
                        <span className="text-gray-600 dark:text-gray-300">
                          (Remarks and photo are optional unless your process
                          says otherwise.)
                        </span>
                      </li>
                    </ul>
                  ...
                  <div>
                    <div className="text:[11px] uppercase tracking-wide text-gray-500 dark:text-gray-400 mb-1">
                      Save Progress (button works when…)
                    </div>
                    <ul className="list-disc pl-5 space-y-1">
                      <li>The WIR is loaded.</li>
                      <li>
                        You’ve entered at least some changes.
                        <br />
                        <span className="text-gray-600 dark:text-gray-300">
                          If a measurement isn’t numeric, you’ll get a prompt
                          and the field is focused for fixing.
                        </span>
                      </li>
                    </ul>
                  </div>
                  ...
                  <div>
                    ...
                    <ul className="list-disc pl-5 space-y-1">
                      ...
                      <li>
                        If any <b>critical</b> item is marked <b>Fail</b>, the
                        recommendation is <b>auto-locked to Reject</b>.
                      </li>
                    </ul>
                  </div>
                  ...
                  <div>
                    <div className="text:[11px] uppercase tracking-wide text-gray-500 dark:text-gray-400 mb-1">
                      Send to HOD (to move from Submitted → Recommended)
                    </div>
                    <ul className="list-disc pl-5 space-y-1">
                      <li>
                        All items have a <b>numeric measurement</b> and a{" "}
                        <b>Pass/Fail</b> status.
                      </li>
                      <li>
                        If anything’s missing, a warning lists the items to
                        complete.
                      </li>
                      <li>
                        You pick a <b>HOD</b> from the derived list (only
                        HOD-capable users are shown).
                      </li>
                      <li>
                        On confirm, the header is updated and the Inspector
                        recommendation is saved. Resulting WIR status becomes{" "}
                        <b>Recommended</b>.
                      </li>
                    </ul>
                  </div>
                  ...
                  <div>
                    <div className="text:[11px] uppercase tracking-wide text-gray-500 dark:text-gray-400 mb-1">
                      HOD tile (the blue “Finalize Now” area)
                    </div>
                    <ul className="list-disc pl-5 space-y-1">
                      <li>
                        <b>Visible</b> only while the WIR is <b>Recommended</b>{" "}
                        (and not already Approved/Rejected).
                      </li>
                      <li>
                        <b>Finalize Now</b> enabled only if you’re acting as{" "}
                        <b>HOD</b> or <b>Inspector+HOD</b>.
                      </li>
                    </ul>
                  </div>
                  ...
                  <div>
                    <div className="text:[11px] uppercase tracking-wide text-gray-500 dark:text-gray-400 mb-1">
                      Finalize Now (Approve/Reject)
                    </div>
                    <ul className="list-disc pl-5 space-y-1">
                      <li>
                        Choose <b>Approve</b> or <b>Reject</b> and (optionally)
                        add a short note.
                      </li>
                      <li>
                        On finalize, WIR moves to <b>Approved</b> or{" "}
                        <b>Rejected</b> and the HOD outcome/notes/time are
                        recorded.
                      </li>
                    </ul>
                  </div>
                  ...
                  <div>
                    ...
                    <ul className="list-disc pl-5 space-y-1">
                      <li>
                        The HOD tile hides, and the <b>HOD Finalized Outcome</b>{" "}
                        summary appears.
                      </li>
                    </ul>
                  </div>
                  ...
                  <div>
                    ...
                    <ul className="list-disc pl-5 space-y-1">
                      <li>
                        Always available in the Runner view to see what will be
                        submitted (no editing there).
                      </li>
                    </ul>
                  </div>
                  ...
                  <div>
                    ...
                    <ul className="list-disc pl-5 space-y-1">
                      <li>
                        <b>Auto-lock to Reject</b> if any <b>critical</b> item
                        is marked <b>Fail</b>.
                      </li>
                      <li>
                        <b>Numeric guard:</b> if a measurement isn’t a number,
                        you’ll be asked to correct it.
                      </li>
                      <li>
                        <b>Missing fields list</b> appears if you try{" "}
                        <b>Send to HOD</b> with incomplete items.
                      </li>
                      <li>
                        If the Runner tab ever becomes hidden (e.g., status
                        changes), the screen auto-returns to <b>Overview</b>.
                      </li>
                    </ul>
                  </div>
                </section>
                ...
                <div className="dark:text-white space-y-1">
                  ...
                  <div>
                    ...
                    {row.rescheduleForDate
                      ...
                      ? new Date(row.forDate).toLocaleDateString()
                      : "—"}{" "}
                    • <b>Time:</b> {row.rescheduleForTime || row.forTime || "—"}
                  </div>
                </div>
              </div>
              <div className="rounded-xl border dark:border-neutral-800 p-3 space-y-3">
                ...
                <ul className="space-y-3">
                  {[...(row.histories || [])]
                    ...
                    .map((h) => (
                      <li
                        key={h.id}
                        className="rounded-2xl border border-slate-200/80 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-4 shadow-sm"
                      >
                        ...
                        <div className="mt-1 text-[12px] text-gray-700 dark:text-gray-300">
                          ...
                          {h.actorName ||
                            (h.actorUserId
                              ? actorNameMap[String(h.actorUserId)]
                              : "") ||
                            h.actorUserId ||
                            "—"}
                        </div>
      ...
      {runnerHistoryOpen && row && (
        ...
        >
          <div className="bg-white dark:bg-neutral-900 rounded-2xl shadow-xl border dark:border-neutral-800 w-[96vw] max-w-4xl max-h-[85vh] overflow-auto">
            ...
            <div className="p-4">
              {(
                row.items?.some((it) => (it.runs?.length || 0) > 0)
                  ? false
                  : true
              ) ? (
                <div className="text-sm text-gray-600 dark:text-gray-300">
                  No runner history recorded.
                </div>
              ) : (
                          ...
                          <ul className="space-y-2">
                            {[...(it.runs || [])]
                              ...
                              .map((r, idx) => (
                                <li
                                  key={idx}
                                  className="rounded-2xl border border-slate-200/80 dark:border-neutral-800 bg-slate-50/60 dark:bg-neutral-800/40 p-3"
                                >
                          ...
                          <div className="text-sm flex items-center gap-2">
                            ...
                            <span
                              className={
                                ...
                                (buf.status === "PASS"
                                  ? "bg-emerald-600 text-white border-emerald-600 hover:bg-emerald-700"
                                  : "bg-white text-slate-700 border-green-200 hover:bg-green-50 hover:border-green-300 dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-100 dark:hover:bg-neutral-800")
                              }
                            >
                              {buf.status ?? "—"}
                            </span>
                              ...
                              <div className="mt-1 flex flex-col gap-1">
                                {(it.evidences || []).map((ev) => (
                                  <div key={ev.id} className="text-[12px]">
                                    ...
                                    <span className="ml-1 text-[10px] px-1 py-0.5 rounded border dark:border-neutral-800">
                                      {ev.kind}
                                    </span>
                                  </div>
                                ))}
                                {buf.evidenceUrl && (
                                  <div className="text-[12px]">
                                    ...
                                    <a
                                      className="underline"
                                      href={buf.evidenceUrl}
                                      target="_blank"
                                      rel="noreferrer"
                                    >
                                      {buf.evidenceName ||
                                        baseName(buf.evidenceUrl) ||
                                        "Attachment"}
                                    </a>{" "}
                                    <span className="ml-1 text-[10px] px-1 py-0.5 rounded border dark:border-neutral-800">
                                      Pending
                ...
                <ul className="space-y-1">
                  {hodList.map((u) => {
                    ...
                    return (
                      <li
                        key={u.userId}
                        className={`flex items-center justify-between rounded border dark:border-neutral-800 px-3 py-2 cursor-pointer ${
        ...
        >
          <div className="bg-white dark:bg-neutral-900 rounded-2xl shadow-xl border dark:border-neutral-800 w-[92vw] max-w-md p-4">
            ...
            <div className="mt-3 text-sm text-gray-700 dark:text-gray-300 space-y-1">
              <div>
                <b>Inspector Recommendation →</b>{" "}
                {hodPlannedPatch.inspectorRecommendation}
              </div>
              {hodPlannedPatch.contractorId && (
                <div>
                  <b>Contractor →</b> {nameOf(hodPlannedPatch.contractorId)}
                </div>
              )}
              <div>
                <b>Inspector →</b> {nameOf(row?.inspectorId)}
              </div>
              <div>
                <b>HOD →</b> {nameOf(hodPlannedPatch.hodId)}
              </div>
              <div>
                <b>BIC →</b> {nameOf(hodPlannedPatch.bicUserId)}
              </div>
            </div>
        ...
        >
          <div className="bg-white dark:bg-neutral-900 rounded-2xl shadow-xl border dark:border-neutral-800 w-[92vw] max-w-md p-4">
            ...
            <div className="mt-2 text-sm text-gray-700 dark:text-gray-300">
              Please enter a numeric value (e.g., <b>10</b> or <b>10.5</b>) and
              try again.
            </div>
            <div className="mt-4 flex justify-end gap-2">
              <button
        ...
        >
          <div className="bg-white dark:bg-neutral-900 rounded-2xl shadow-xl border dark:border-neutral-800 w-[92vw] max-w-md p-4">
            ...
            <div className="mt-2 text-sm text-gray-700 dark:text-gray-300">
              ...
              <b>Pass/Fail</b> status, a numeric <b>Measurement</b> if tagged{" "}
              <i>measurement</i>, and an attached <b>Photo/Document</b> if
              tagged <i>evidence/document</i>.
            </div>
      ...
      {noEditPermOpen && (
        <div
          className="fixed inset-0 z-[105] flex items-center justify-center bg-black/40"
          role="dialog"
          aria-modal="true"
        >
          <div className="bg-white dark:bg-neutral-900 rounded-2xl shadow-xl border dark:border-neutral-800 w-[92vw] max-w-md p-4">
            <div className="text-base font-semibold dark:text-white">
              Runner — Edit Not Allowed
            </div>
            <div className="mt-2 text-sm text-gray-700 dark:text-gray-300">
              Editable Runner opens only when{" "}
              <b>Inspector + Submitted + BIC is you</b>. You currently don’t
              meet this condition.
            </div>
            <div className="mt-4 flex justify-end">
              <button
            ...
            <section className="mt-3 rounded-xl border dark:border-neutral-800 p-3 space-y-3">
              ...
              <div>
                ...
                <textarea
                  value={finalizeNote}
                  onChange={(e) =>
                    setFinalizeNote(e.target.value.slice(0, 200))
                  }
                  rows={3}
                  className="w-full text-sm px-3 py-2 rounded-lg border dark:border-neutral-800 bg-white dark:bg-neutral-900"
            ...
            <section className="mt-3 rounded-xl border dark:border-neutral-800 p-3 space-y-3">
              ...
              <div className="rounded-lg border dark:border-neutral-800 p-3">
                <div className="text-[12px] font-semibold mb-1">
                  Phase 1 — header patch (single PATCH)
                </div>
                {mappedFinalizeOutcome ? (
                  ...
                  <div className="text-[12px] text-gray-600 dark:text-gray-400">
                    Pick <b>Accept</b> or <b>Reject</b> to preview the exact
                    payload.
                  </div>
                )}
                <ul className="mt-2 list-disc pl-5 text-[12px] text-gray-700 dark:text-gray-300 space-y-1">
                  <li>
                    <code>hodOutcome</code> = <code>"APPROVE"</code> for Accept,{" "}
                    <code>"REJECT"</code> for Reject (mapped from local{" "}
                    <code>"ACCEPT"</code> | <code>"REJECT"</code>).
                  </li>
                  <li>
                    <code>hodDecidedAt</code> = current ISO timestamp.
                  </li>
                  <li>
                    <code>hodRemarks</code> included only if note is non-empty
                    (trimmed).
                  </li>
                  <li>
                    <code>bicUserId</code> reassigned per rules:
                    <ul className="list-disc pl-5 mt-1">
                      <li>
                        APPROVE + Inspector <code>APPROVE_WITH_COMMENTS</code> →
                        contractorUid (fallback: <code>contractorId</code>, else{" "}
                        <code>createdById</code>, else <code>null</code>)
                      </li>
                      <li>
                        APPROVE + Inspector plain <code>APPROVE</code> →{" "}
                        <code>null</code>
                      </li>
              ...
              <div className="rounded-lg border dark:border-neutral-800 p-3">
                <div className="text-[12px] font-semibold mb-1">
                  Phase 2 — status patch (separate PATCH)
                </div>
                {mappedFinalizeOutcome ? (
                  <pre className="text-[11px] overflow-auto whitespace-pre-wrap leading-5 bg-gray-50 dark:bg-neutral-800 dark:text-gray-100 p-2 rounded">
                ...
                <ul className="mt-2 list-disc pl-5 text-[12px] text-gray-700 dark:text-gray-300 space-y-1">
                  <li>
                    If Accept → <code>{'{ status: "Approved" }'}</code>
                  </li>
                  <li>
                    If Reject → <code>{'{ status: "Rejected" }'}</code>
                  </li>
                </ul>
              </div>

              <div className="text-[12px] text-gray-700 dark:text-gray-300">
                Then the app refreshes the document via <code>fetchWir()</code>,
                closes the modal, and clears local finalize state.
              </div>
            </section>
        ...
        <FollowupScheduleModal
          ...
          onCreated={(newWirId?: string) => {
            ...
            if (newWirId) {
              ...
              const project = (loc.state as NavState | undefined)?.project || {
                projectId,
              };
